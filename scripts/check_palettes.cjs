// Register ts-node without loading the project's tsconfig to avoid
// compilation errors when running this helper script.
const fs = require('fs');
const path = require('path');

// Parse PALETTES from src/lib/palettes.ts without requiring TypeScript modules.
const src = fs.readFileSync(path.join(__dirname, '..', 'src', 'lib', 'palettes.ts'), 'utf8');

function parsePalettes(source) {
    const palettes = [];
    // Split by palette object blocks starting with '{' that contain 'id:'
    const blocks = source.split(/\{\s*\n/).slice(1);
    for (const block of blocks) {
        const idMatch = block.match(/id:\s*(\d+)/);
        if (!idMatch) continue;
        const nameMatch = block.match(/name:\s*'([^']+)'/);
        const colorsMatch = block.match(/colors:\s*\[([^\]]+)\]/m);
        const id = Number(idMatch[1]);
        const name = nameMatch ? nameMatch[1] : `palette-${id}`;
        const colors = colorsMatch
            ? colorsMatch[1].match(/#[0-9A-Fa-f]{6}/g) || []
            : [];
...
function compositeOver(baseHex, overlayHex, alpha) {
